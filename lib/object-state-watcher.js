...
function ObjectStateWatcher() {
  ...
  this.on(ObjectStateWatcher.Signals.initializationSucceed, () => this.currentState = ObjectStateWatcher.States.Ready);
}
...
const _handleInitializationOptionsSchema = Joi.object().keys({
  objects: Joi.array().items(
    Joi.object().keys({
      ...
      removeListener: Joi.func().required()
    }).unknown()
  ).required(),
...
exports.ObjectStateWatcher = module.exports.ObjectStateWatcher = ObjectStateWatcher;
