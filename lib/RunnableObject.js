...
RunnableObject.prototype.start = function () {
  ...
  return new Promise((resolve, reject) => {
    if (this.currentState !== RunnableObject.States.Ready) {
      ...
      setImmediate(reject, new IllegalStateException(util.format("Start is not permitted with state %s", this.currentState)));
    } else {
...
RunnableObject.prototype.stop = function () {
  ...
  return new Promise((resolve, reject) => {
    if (_.indexOf([
      ...
      setImmediate(reject, new IllegalStateException(util.format("Stop is not permitted with state %s", this.currentState)));
    } else {
...
RunnableObject.prototype.pause = function () {
  ...
  return new Promise((resolve, reject) => {
    if (!this.canPause()) {
      ...
      setImmediate(reject, new OperationNotPermittedException(util.format("Pause is not permitted")));
      return;
    }
    ...
    if (this.currentState === RunnableObject.States.Paused || this.currentState === RunnableObject.States.Pausing) {
      ...
      setImmediate(reject, new IllegalStateException(util.format("Pause is not permitted with state %s", this.currentState)));
    } else {
...
RunnableObject.prototype.resume = function () {
  ...
  return new Promise((resolve, reject) => {
    if (!this.canPause()) {
      ...
      setImmediate(reject, new OperationNotPermittedException(util.format("Resume is not permitted")));
      return;
    }
    ...
    if (this.currentState === RunnableObject.States.Running || this.currentState === RunnableObject.States.Resuming) {
      ...
      setImmediate(reject, new IllegalStateException(util.format("Resume is not permitted with state %s", this.currentState)));
    } else {
